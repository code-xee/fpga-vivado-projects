...
int main(){
	...
	status = XAxiDma_SimpleTransfer(&myDma, (u32)b, 8*sizeof(u32),XAXIDMA_DEVICE_TO_DMA);// S2MM





	status = XAxiDma_SimpleTransfer(&myDma, (u32)a, 8*sizeof(u32),XAXIDMA_DMA_TO_DEVICE);// MM2S
	if(status != XST_SUCCESS){
		print("DMA initialization failed\n");
		return -1;
	}
